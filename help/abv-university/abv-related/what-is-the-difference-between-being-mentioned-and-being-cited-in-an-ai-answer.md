---
title: 人工智能答案中的提及次数与引用次数
description: 提及内容会在文本中命名您的品牌；引文会将您的页面链接为源。 了解为何引文来自检索并且是更强的可见性信号。
feature: Brand Presence
role: Admin, Leader, User
level: Beginner
doc-type: Value Video
duration: 175
last-substantial-update: 2026-09-02T00:00:00Z
jira: KT-22523
source-git-commit: 823cfd29f7816f485995fcb924740a402993fa54
workflow-type: tm+mt
source-wordcount: '162'
ht-degree: 0%

---

# 在人工智能答案中被提及和被引用之间有何区别？

提及内容会在文本中命名您的品牌；引文会将您的页面链接为源。 此视频解释了不同之处以及引文为更强可见性信号的原因。

## 此视频面向谁？

* 营销人员和用户解释AI如何代表其品牌
* 团队将品牌提及与实际引文区分开来

## 视频内容

在本视频中，您将了解：

* 这里要说的是：答案文本中提到您的品牌
* 引文是什么：指向您的页面的链接，作为支持来源
* 为何提及可能来自训练数据，而无需检索
* 检索(RAG)是如何产生真实引文的
* 为什么引用是表明你的品牌被用作证据的更强烈的信号

>[!VIDEO](https://video.tv.adobe.com/v/3502765/?learn=on){transcript=true}